...
-- Recreate create_order_secure with correct insertion order:
-- Insert order first, then items (respects FK constraint).
-- Also adds region-aware pricing support for Dubai (price_aed).
...
BEGIN
  ...
  -- First pass: compute subtotal from DB prices
  ...
  -- Then insert items (child rows referencing the order)
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items)
  LOOP
    SELECT price, name INTO v_product_price, v_product_name
    FROM public.products
